...
export async function createEvent({ creator, name, slug, description } = {}) {
  const q = `
    INSERT INTO events
      (creator, name, slug, description)
    VALUES
      ($1, $2, $3, $4)
    RETURNING id, creator, name, slug, description;
  `;
  const values = [creator, name, slug, description];
...
export async function listEvents() {
  const q = `
    SELECT
      id, creator, name, slug, description, created, updated
    FROM
      events
  `;
...
export async function deleteQuery(q, values = []) {
  const result = await query(q, values);
